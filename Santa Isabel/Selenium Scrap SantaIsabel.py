...
def get_data_product(browser, id_category, id_brand, id_type, id_product, columns):
    ...
    print('id_brand: ', id_brand)
    print('id_type: ', id_type)
    print('name_product: ', name_product)
    print('image_product: ', image_product)
    print('description_product: ', description_product)
...
def get_data_categories(df_categories, categories_element):   
    ...
    for item in categories_element:
        if item.get('href') != '/' and exists_row_dataframe(df_categories, 'category', item.text) == False:
            df_row = pd.DataFrame(data=[[id_category, (item.text).upper()]], columns=columns_category)
...
def get_data_superproduct(browser, id_supermarket, id_product, url, date):
    ...
    print('----- SUPER PRODUCT -----')
    print('id_supermarket', id_supermarket)
    print('id_product', id_product)
    print('sale_price', sale_price)
    print('normal_price', normal_price)
    print('url', url)
    print('date', date)
    print('stock', stock)
    print('-------------------------')
...
columns_type = ['id_type', 'name_type']
...
for i in range(len(list_url_categories)):
    ...
    for url_product in list_url_products:
        ...
        brand_product = get_text_selenium(driver, '//*[@id="root"]/div/div[2]/div/div/main/div[1]/div[2]/div[3]/div[1]/a')
        type_product = get_text_selenium(driver, '//*[@id="root"]/div/div[2]/div/div/main/div[4]/div[1]/div[2]/div/div[2]/span')
        ...
        if exists_row_dataframe(df_type, 'name_type', type_product) == False:
        ...
        # Get product details
        data_product = get_data_product(driver, id_brand, id_type, id_category, id_product, columns_product)
        if data_product == 'NA':
        ...
        if exists_superproduct(df_super_product, id_supermarket, id_product, date_scrap) == False:
            data_superproduct = get_data_superproduct(driver, id_supermarket, id_product, url_product, date_scrap)
        ...
        print('[{}/{}] > {} - ${}'.format(cont_products_category, total_products_category, data_product[4], data_superproduct[3]))
